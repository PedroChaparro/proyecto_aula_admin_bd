...
CREATE PROCEDURE register_new_client(
	...
	/*Encriptar la contraseña*/ SET @hashed = SHA2(contraseña, 256);
...
WHERE usuarios.id_usuario = id_usuario; END// 
DELIMITER ;
...
CREATE PROCEDURE update_user(
	...
	/*Encriptar la contraseña*/ SET @hashed = SHA2(contraseña_actual, 256); 
...
WHERE usuarios.id_usuario = id_usuario; SET @success = 1; END IF;
SELECT @success; END//
...
WHERE usuarios.correo_electrónico = correo_electrónico; 
	...
	IF @user_exists = 1 THEN 
		...
		/*Encripta la contraseña para compararla con la que está en la base de datos*/ SET @hashed = SHA2(contraseña, 256);
SELECT usuarios.contraseña INTO @saved_password
FROM usuarios
WHERE usuarios.correo_electrónico = correo_electrónico; 
		
		IF @saved_password = @hashed THEN SET @success = 1; END IF; END IF; 
