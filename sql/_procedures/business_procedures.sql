...
FROM VEHÍCULOS
WHERE VEHÍCULOS.id_vehículo = id_vehículo; SET @semanas =
...
DELIMITER //
CREATE PROCEDURE get_rental_history() BEGIN
SELECT *
FROM rental_data_history_pretty; END //
...
WHERE alquileres.id_alquiler = id_alquiler; END//
...
WHERE `VEHÍCULOS`.`id_vehículo` = @vehiculo; END // 
...
WHERE a.id_alquiler = id_alquiler; 
	...
	IF (@dias_mora > 0) THEN 
		/*Cálculo del 8% de recargo*/ SET @interes = (@dias_mora * @freezed_daily) * 0.08; SET @recargo = (@dias_mora * @freezed_daily) + @interes; END IF; 	
...
SELECT *
FROM factura AS f
WHERE f.id_alquiler = id_alquiler; END //
...
WHERE factura.id_factura = id_factura; END//
